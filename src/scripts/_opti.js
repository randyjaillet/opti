...
Opti.prototype.init = function () {
	...
	this.o = $('<div/>')
			.addClass("opti")
			.attr("id", root.s.attr("id") ? `${root.s.attr("id")}-opti` : null)
			.attr("data-theme", root.s.data("theme"))
			.attr("data-scheme", root.s.data("scheme"))
			.addClass(root.settings.classes)
	;
	...
	this.s.find("optgroup").attr("data-groupindex", () => root.groupCount++);
	this.convertOptsAndGroups(this.s).appendTo($oList);
	...
	if ($initiallySelectedOpts.length) {
		...
		if (ufoapBool) {
			...
			it here. */
			this.applyPlaceholderIfApplicable();
	...
	*/
	...
	this.l.length && this.l.on(
		...
		e => {
			...
			oIsDisabled = root.o.is("[disabled=disabled]");
			if (!oIsDisabled & !inOpti) root.showMenu();
		}
	);
	...
	this.s.on({
		...
		change: (e, ignore) => {
			if (!ignore) {
				...
				root.unchooseOption($liTargetsToUnchoose, false, false, false, true);
				root.chooseOption(sVal);
			}
		}
	...
	$oSurface.on(
		...
		e => {
			e.preventDefault();
			oIsDisabled = root.o.is("[disabled=disabled]");
			if (!oIsDisabled) root.o.is(".activated") ? root.hideMenu() : root.showMenu();
		}
	);

	this.o.on({
		focusin: () => {
			root.o.addClass("focus-in");
		},
		focusout: () => {
			root.o.removeClass("focus-in");
		}
	});
	...
	$oSurface.on({
		...
		keypress: e => {
			...
			var oIsActivated = root.o.hasClass("activated");
			oIsDisabled = root.o.is("[disabled=disabled]");
			...
			if (!oIsDisabled) {
				...
				if (e.which == 32) {
					e.preventDefault();
					oIsActivated ? root.hideMenu(false) : root.showMenu();
				}
		...
		keydown: e => {
			$oOpts = $("span", $oList);
			oIsDisabled = root.o.is("[disabled=disabled]");
			oIsMultiple = root.o.is("[multiple=multiple]");
			const
				isCurrentlyActivated = root.o.hasClass("activated"),
			...
			switch(e.which) {
				...
				case 27:
					e.preventDefault();
					isCurrentlyActivated && root.hideMenu();
					break;
				...
				case 9:
					if ($currentlyFocused.length && !$currentlyFocused.is($currentlySelected) && !oIsDisabled && isCurrentlyActivated && !oIsMultiple) {
						root.chooseOption($currentlyFocused);
					}
					root.hideMenu();
					break;
					...
					if (!oIsDisabled) {
						...
						if (isCurrentlyActivated) {
							...
							if ($currentlyFocused.length) {
								...
								// If an option is fake focused...
								// (If none are, we don't care about enter key presses.)
								if ($currentlyFocused.length) root.selectionAction($currentlyFocused);
								break;
				...
				case 38:
					if (isCurrentlyActivated) {
						...
						if (!oIsMultiple) {
							e.preventDefault();
							const shouldWeFade = !root.isBlankOrPlaceholder();
							const $optionTarget = $CSPrevAll.length ? $CSPrevAll.last() : $selectableOpts.last();
							root.chooseOption($optionTarget, false, shouldWeFade);
						}
					}
					break;
	...
	$oSearchInput.on({
		keydown: e => {
			...
			switch(e.which) {
				...
				case 36:
					...
					option in the list */
					if ($selectableOpts.length) root.fakeFocusOn($selectableOpts.first());
					break;
				...
				case 35:
					...
					/* Move fake focus to the first
					option in the list */
					if ($selectableOpts.length) root.fakeFocusOn($selectableOpts.last());
					break;
				...
				case 27:
					e.preventDefault();
					root.o.hasClass("activated") && root.hideMenu();
					break;
		...
		"input change": e => {
			if (this.o.find(".search input").val()) {
				root.searchMenu(this.o.find(".search input").val());
			} else {
				root.unsearchMenu();
			}
		}
	...
	$oSurfTextUL.on(
		...
		e => {
			...
			const
				...
				nothingsDisabled = !($tag.is("[disabled=disabled]") || root.o.is("[disabled=disabled]"))
			;
			if (nothingsDisabled) root.unchooseOption($tag.attr("data-value"), false, false, $oSurface);
		}
	);
	...
	$oSurfIconEx.on({
		click: e => {
			...
			const
				oIsNotDisabled = root.o.is(":not([disabled=disabled])"),
				$targets = root.o.find(".list .selected").not("[disabled=disabled]")
			;

			/* Unchoose everything and focus on the surface. */
			if (oIsNotDisabled) root.unchooseOption($targets, false, false, $oSurface);
		},
		mouseenter: e => {
	...
	$(document).on(
		...
		e => {
			var
				clickIsInOpti = $(e.target).closest(root.o).length,
				clickIsInLinkedLabel = $(e.target).closest(`[for=${root.s.attr("id")}]`)
						.add($(e.target).closest("label").has(root.s))
						.length
			;
			...
			if (mutation.type == "attributes") {
				...
				if ($(mutation.target).is("select")) {
					...
					if (mutation.attributeName == "disabled") {
						if (root.s.attr(mutation.attributeName) != mutation.attributeName) {
							root.o.removeAttr(mutation.attributeName);
						} else {
							root.o.attr(mutation.attributeName, root.s.attr(mutation.attributeName));
						}
					}
					
					if (mutation.attributeName == "multiple") {
						const
							currentVal = root.s.val(),
							targetVal = $.isArray(currentVal) ? currentVal[0] : currentVal,
							$targetOpt = root.o.find(`.list span[data-value="${targetVal}"]`),
							$nonDisabledOpts = root.o.find(".list span:not([disabled=disabled])"),
							callbackArgs = {
								$targetOpt : $targetOpt,
						...
						if (root.o.find(".list span.selected").length) {
							/* If there is anything to unchoose,
							delay choosing until we've unchosen */
							root.unchooseOption(root.o.find(".list span.selected"), chooseOptsNowThatUnchosen, callbackArgs, false, false, false);
						} else {
							/* If there is nothing to unchoose,
							immediately choose */
							chooseOptsNowThatUnchosen(root, callbackArgs);
						}
					}
					...
					if (mutation.attributeName == "value") {
						const
							$correspondingListItem = root.o.find(`.list span[data-value="${mutation.oldValue}"]`),
							$correspondingTag = root.o.find(`.surface .tag[data-value="${mutation.oldValue}"]`)
						;
						...
						const
							...
							$correspondingListItem = root.o.find(`.list span[data-value="${$(mutation.target).val()}"]`)
						;
						if (newVal == "disabled") {
							$correspondingListItem.attr("disabled", "disabled");
							if ($correspondingListItem.is(".selected")) {
								root.unchooseOption($correspondingListItem, () => {
									if (root.isBlank() && !root.placeholderTextEnabled && root.settings.firstOptDefault) {
										root.chooseOption(root.o.find(".list span:not([disabled=disabled])").first());
									} else {
										root.handleZeroStateText();
									}
								}, false, false, true);
				...
				if (mutation.addedNodes.length) {
					...
					mutation.addedNodes.forEach(
						function (currentValue, currentIndex, listObj) {
							const
								$list = root.o.find(".list"),
								$addedNode = $(currentValue),
								$newTree = root.convertOptsAndGroups($addedNode),
							...
							if (addedNodeIsOptgroup) {
								$addedNode.attr("data-groupindex", () => root.groupCount++);
								const
									$sValidAll = root.s.children(`option:not([value="${root.placeholderValue}"]), optgroup`),
								...
								if ($addedNode.is(":first-child")) {
									if (optAddedToOptgroup) {
										const
											opGrpInd = root.s.find("optgroup").index($addedNodeParent),
									...
									const
										$sValidOpts = root.s.find(`option:not([value="${root.placeholderValue}"])`),
					...
					root.isInShortMode = $("span", $oList).length < root.settings.shortModeThreshold;
					if (root.isInShortMode) $oSearch.hide();
					...
					mutation.removedNodes.forEach(
						function (currentValue, currentIndex, listObj) {
							...
							;
							if (removedNodeIsOpt) {
								$nukeTarget = root.o.find(`.list [data-value="${$(currentValue).val()}"]`);
							} else if (removedNodeIsOptgroup) {
								const opGrpInd = $(currentValue).attr("data-groupindex");
								$nukeTarget = root.o.find(`.list section[data-groupindex=${opGrpInd}]`);
							}
							if ($selectedOptsToUnchoose.length) {
								var $listItemTargets = $();
								$selectedOptsToUnchoose.each(
									function (i, v) {
										$listItemTargets = $listItemTargets.add(`span[data-value="${$(v).val()}"]`, root.o.find(".list"));
									}
								);
								root.unchooseOption($listItemTargets, false, true);
							}
							$nukeTarget.remove();
						}
					);
					root.isInShortMode = $("span", $oList).length < root.settings.shortModeThreshold;
					if (!root.isInShortMode) $oSearch.show();
				}
			}
		}
	};
...
Opti.prototype.showMenu = function (focusOnSearch = !this.isInShortMode) {
...
Opti.prototype.hideMenu = function (focusOnSurface = true) {
...
Opti.prototype.orientDropdown = function () {
...
:contains(). It's below. */
Opti.prototype.searchMenu = function (string) {
...
Opti.prototype.unsearchMenu = function () {
 ...
 */
Opti.prototype.chooseOption = function (vals, setFocus, noFade = false) {
	...
	const
		root = this,
		$pht = this.o.find(".surface .text-ph"),
		$txt = this.o.find(".surface .text-op"),
	...
	if ($.isArray(vals)) {
		...
		$.each(
			vals,
			(ind, currVal) => {
				const $target = root.o.find(`.list [data-value="${currVal}"]`);
				$options = $options.add($target);
			}
		);
	} else if (typeof vals == "string") {
		/* A single value was passed in. */
		const $target = root.o.find(`.list [data-value="${vals}"]`);
		$options = $options.add($target);
	...
	if ($options.length) {
		...
		root.unchoosePlaceholderOption();
		...
		if (this.s.is("[multiple=multiple]")) {
			...
			root.s.val($option.attr("data-value"));
		...
		if (wasPlaceholder && !noFade) {
			...
			root.fadeOut($pht, guts, false, fadeOutCallbackArgs);

		} else if (wasBlank || noFade || root.o.is("[multiple=multiple]")) {
			root.o.removeClass("zerostate");
			guts(root, fadeOutCallbackArgs);
		} else {

			root.fadeOut($txt, guts, false, fadeOutCallbackArgs);
...
function guts (root, args) {
	...
	args.$options.each(
		(i, v) => {
			...
			if (!alreadyChosen) {
				...
				let valPosition = $.inArray($(v).attr("data-value"), root.s.val());
				if (args.wasPlaceholder) valPosition = 0;
				const isAtBeginning = valPosition == 0;

				if (isAtBeginning) {
					$newTag.prependTo(root.o.find(".surface ul"));
				} else {
					const $targTag = root.o.find(".surface .tag").eq(valPosition - 1);
	...
	// Multiple-select Optis
	if (root.o.is("[multiple=multiple]")) {
		...
		root.o.find(".search input").val(null).trigger("change");

		if (args.setFocus) {
			const
				$oSearch = root.o.find(".search"),
				$oSearchInput = $oSearch.find("input"),
				$oSurf = root.o.find(".surface"),
				$focusTarget = root.isInShortMode && $oSearch.is(":hidden") ? $oSurf : $oSearchInput
			;
			$focusTarget.trigger("focus");
		}
	}
	...
	// Single-select Opti
		...
		root.o.find(".surface .text-op").text($option.text());

		if ($optParent.is("section:not(.list)")) {
			root.o.find(".surface .text-op").prepend(`<span class="text-grp">${$optParent.find("h5").text()}</span>`);
		}
	...
	external to Opti. */
	root.s.add(root.o).trigger("change", true);
...
un-chosen. */
Opti.prototype.unchooseOption = function (vals, callback, callbackArgs, focusTarget, dontUpdateSelect, noFade = false) {
	...
	$options.each(
		(i, v) => {
			$targetTags = $targetTags.add($(`.surface ul .tag[data-value="${$(v).attr("data-value")}"]`, root.o));
		}
	);
	...
	if (noFade || (!$targetTags.length && oIsMulti)) {
		...
		if (typeof callback === "function") {
			callback(root, callbackArgs);
		}
		root.handleZeroStateText();
		
	} else if (oIsMulti && $targetTags.length) {

		this.fadeOutTag($targetTags, callback, callbackArgs);

	} else {

		this.fadeOut($oSurface.find(".text-op"), callback, false, callbackArgs);
...
function chooseOptsNowThatUnchosen (root, args) {
	if (root.s.attr("multiple") != "multiple") {
		root.o.removeAttr("multiple");
	} else {
		root.o.attr("multiple", "multiple");
	}
	if (args.$targetOpt.length) {
		root.chooseOption(args.$targetOpt, false, true);
	} else {
		if (root.isBlank() && !root.placeholderTextEnabled && root.settings.firstOptDefault && args.$nonDisabledOpts.length) {
			root.chooseOption(args.$nonDisabledOpts.first(), false);
		} else {
			root.handleZeroStateText();
		}
	}
...
Opti.prototype.applyPlaceholderIfApplicable = function () {
...
Opti.prototype.unchoosePlaceholderOption = function () {
...
Opti.prototype.fadeOut = function ($t, callback, removeTarget, callbackArgs) {
...
Opti.prototype.fadeOutTag = function ($t, callback, callbackArgs) {
...
Opti.prototype.handleZeroStateText = function () {
...
Opti.prototype.fakeFocusOn = function ($t) {
...
inadequate adoption. */
Opti.prototype.scrollOptionIntoView = function ($option) {
...
Opti.prototype.isBlankOrPlaceholder = function () {
...
Opti.prototype.isBlank = function () {
	const
		val = this.s.val(),
...
Opti.prototype.isPlaceholder = function () {
...
Opti.prototype.convertOptsAndGroups = function ($tree) {
...
Opti.prototype.selectionAction = function ($t) {
...
Opti.prototype.destroy = function (preserveSlotInWindowArray) {
